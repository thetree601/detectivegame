...
async function migrateUserProgress() {
  ...
  try {
    ...
    if (
      ...
      console.log("\n🔧 연결 오류 해결 방법:");
      console.log(
        "   1. Supabase Dashboard → Settings → Database에서 연결 문자열 확인"
      );
      console.log(
        "   2. 비밀번호에 특수문자가 있으면 URL 인코딩 필요 (예: @ → %40)"
      );
      console.log(
        "   3. 또는 Supabase의 직접 DB 연결이 제한되어 있을 수 있습니다"
      );
      console.log(
        "   4. 이 경우 아래 SQL을 Supabase Dashboard의 SQL Editor에서 직접 실행하세요\n"
      );
    } else {
      console.log(
